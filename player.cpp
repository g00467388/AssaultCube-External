...
void Player::setYaw(float yaw) {
	WriteProcessMemory(_gameHandle, (LPVOID)(_playerBaseAddress + offsets::yaw), &yaw, sizeof(yaw), nullptr);
}

void Player::setPitch(float pitch)
{
	WriteProcessMemory(_gameHandle, (LPVOID)(_playerBaseAddress + offsets::pitch), &pitch, sizeof(pitch), nullptr);
}

int32_t Player::getAmmo()
{
	ReadProcessMemory(_gameHandle, (LPCVOID)(_playerBaseAddress + offsets::ammo), &_ammo, sizeof(_ammo), nullptr);
	return _ammo;
}

void Player::setAmmo(uint32_t amount)
{
	WriteProcessMemory(_gameHandle, (LPVOID)(_playerBaseAddress + offsets::ammo), &amount, sizeof(amount), nullptr);
}
